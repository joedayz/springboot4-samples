...
Vuelve a la terminal donde la aplicación está ejecutándose y verifica que aparece un log en `ERROR` con el mensaje de `Expense not found: none` (el logger y formato exacto pueden variar por configuración).
...
Envíale los logs a un archivo, define un formato de logging y desactiva la rotación del archivo al reiniciar.
