...
```bash
podman build -f src/main/podman/Dockerfile.jvm -t expense-function:jvm .
```
...
```bash
podman build -f src/main/podman/Dockerfile.native -t expense-function:native .
```
...
- Integra con AWS Lambda o Google Cloud Functions
